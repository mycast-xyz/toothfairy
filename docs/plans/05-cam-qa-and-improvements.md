...
> 커밋: 프론트 `8923dfa`·`e25ad0d`·`9b9256a`, 백엔드 `6f6c058`.
> **미결정(사용자 확인 필요)**: **A4** 취소 버튼 — 소켓서버에 stopPrint 핸들러가 없어(死버튼 확정) 완료를 되돌리는 기능이 없다. **① 버튼 제거** vs **② 되돌리기 REST/소켓 구현**(백엔드 신규) 중 방향 결정 필요(O-4).
