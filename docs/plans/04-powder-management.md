...
> 작성 2026-07-01 · 상태: **Phase 1·2 구현 완료**
>
> ⚠️ **스트림 정정(2026-07-01)**: 파우더 2종은 **캡** / **파샬+올온포** (기존 문서의 캡·파샬 / 올온포에서 사용자 정정). 구현 반영: `cap` / `partialAllonfour`.
>
> **Phase 2 완료(커밋 프론트 `2caae47`·백엔드 `2f67663`)**: 남은량·보충 입력 + 소모량/유닛당 계산 + 통용량 설정.
> - 백엔드: `powder_daily`(남은량/보충)·`powder_config`(통용량) 테이블(`sql/schema/powder.sql`) + `GET /list`·`POST /record`·`PUT /config`(인증).
> - 프론트: 통용량 설정(스트림별) + 일자별 입력 + 소모량(=전날 남은량+보충통수×통용량−오늘 남은량)·유닛당(g/유닛) 계산 테이블. 첫 기록일·유닛0은 계산 불가(—).
> - **통 용량은 UI 설정**(기본 1000g, 스트림별 저장). 유닛=정상+리메이크, 커스텀 제외.
> - 검증: 06-01 10000 → 06-02 9200 = 소모량 800g, g/유닛 12.7(800/63).
>
