<script lang="ts">
	...
	const validateId = async (id: string) => {
		const idRegex = /^[a-zA-Z0-9_-]{4,20}$/; // 4~20자, 영문/숫자/언더스코어/하이픈 허용
		...
		if (!idRegex.test(id)) {
			emailError =
				'아이디는 4~20자의 영문, 숫자, 언더스코어(_) 또는 하이픈(-)만 사용할 수 있습니다.';
			isEmailValid = false;
			return false;
		}
		// 아이디 중복 확인을 위한 서버 요청
		try {
			const response = await fetch(`${currentUrl}/api/v0/account/validate/id`, {
				method: 'POST',
				headers: {
			...
			if (!response.ok) {
				...
				emailError = data.resultMsg || '아이디 확인 중 오류가 발생했습니다.';
				isEmailValid = false;
	...
	const handleSubmit = async () => {
		...
		if (!validateId(data.id as string)) {
			return;
		}
		// 비밀번호 검증
		if (!data.password) {
			...
			<form class="flex flex-col gap-4" onsubmit={handleSubmit} id="createForm">
				<div>
					...
					<input
						...
						oninput={(e) => validateId(e.currentTarget.value)}
					/>
					{#if emailError}
				...
				<div>
					<label for="email" class="mb-1 block text-sm font-medium">이름</label>
					<input
						type="text"
						id="name"
						name="name"
