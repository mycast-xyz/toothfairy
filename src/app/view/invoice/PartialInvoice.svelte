<script lang="ts">
	...
	// 일반/리메이크 금액 계산
	corpInfo.totalNormalPrice = corpInfo.totalNormalUnitNum * corpInfo.prices.partial.normal;
	corpInfo.totalRemakePrice = corpInfo.totalRemakeUnitNum * corpInfo.prices.partial.remake;

	// 청구서 가격 계산 함수
	function getInvoicePrices(invoice: any, type: string) {
		if (type === 'normal') {
			return invoice * corpInfo.prices.partial.normal;
		} else if (type === 'remake') {
			return invoice * corpInfo.prices.partial.remake;
		}
	}
				...
				<tbody>
					{#each dateInfo as item}
						...
						>
							...
							>
								{#if item.invoice?.directory}
									<p
										class=" max-w-[500px] whitespace-normal break-words text-xs font-normal text-gray-500 dark:text-gray-400"
									>
										{#each getFileInfo(item.invoice.directory.remakeFiles, item.invoice.corpName).types as type, i}
											{type}{i <
											getFileInfo(item.invoice.directory.remakeFiles, item.invoice.corpName).types
												.length -
												1
												? ', '
												: ''}
